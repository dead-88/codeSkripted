...
try:
    ...
    print "\t#              / __/ /__ ____(_)__  / /____ ___/ /             #"
    print "\t#             _\ \/  '_// __/ / _ \/ __/ -_) _  /              #"
    print "\t#            /___/_/\_\/_/ /_/ .__/\__/\__/\_,_/               #"
    print "\t#                         /_/                                  #"                                             
    print "\t#                                                              #"
    print "\t#                                                              #"
    print "\t#                                                              #"
...
'adm/index.brf','adm.brf','affiliate.brf','adm_auth.brf','memberadmin.brf','administratorlogin.brf']
    ...
    try:
        site = raw_input("Sitio A Escanear?: ")
        site = site.replace("http://","")
        print ("\tComprobando sitio web " + site + "...")
        conn = httplib.HTTPConnection(site)
        conn.connect()
        print "\t[$] Si... El servidor esta en linea :D ."
    except (httplib.HTTPResponse, socket.error) as Exit:
        raw_input("\t [!] Oops a ocurrido un error, el servidor esta offline")
        exit()
    print "Introduzca el codigo fuente del sitio:"
    print "1 PHP"
    print "2 ASP"
    print "3 CFM"
    print "4 JS"
    ...
    if code==1:
        ...
        for admin in php:
            ...
            if response.status == 200:
                var1 = var1 + 1
                print "%s %s" % ( "\n\n>>>" + host, "Admin page Encontrada!")
                raw_input("Presiona enter para continuar el escaner...\n")
            elif response.status == 404:
                var2 = var2
            elif response.status == 302:
                print "%s %s" % ("\n>>>" + host, "Posible admin page (302 - Redirect)")
            else:
                print "%s %s %s" % (host, " Interesting response:", response.status)
            connection.close()
        print("\n\nCompletado \n")
        print var1, " paginas de admistracion encontradas"
        print var2, " total de paginas escaneadas"
        raw_input("[/] Fin del juego, presiona una tecla para continuar")
    ...
    if code==2:
        ...
        for admin in asp:
            ...
            if response.status == 200:
                var1 = var1 + 1
                print "%s %s" % ( "\n\n>>>" + host, "Pagina de administracion encontrada!")
                raw_input("Presiona una tecla para continuar el escaner.\n")
            elif response.status == 404:
                var2 = var2
            elif response.status == 302:
                print "%s %s" % ("\n>>>" + host, "Posible pagina de admistracion(302 - Redirect)")
            else:
                print "%s %s %s" % (host, " Interesting response:", response.status)
            connection.close()
        print("\n\nCompleted \n")
        print var1, " paginas de administracion encontradas"
        print var2, " total de paginas escaneadas"
        raw_input("Fin del juego, presiona una tecla para continuar")
    ...
    if code==3:
        ...
        for admin in cfm:
            ...
            if response.status == 200:
                ...
                print "%s %s" % ("\n>>>" + host, "Posible pagina de administracion (302 - Redirect)")
            else:
                print "%s %s %s" % (host, " Interesting response:", response.status)
            connection.close()
        print("\n\nCompleted \n")
        print var1, " paginas de administracion encontradas"
        print var2, " total de pagina escaneadas"
        raw_input("Fin del juego, presiona una tecla para continuar")
    ...
    if code==5:
        ...
        for admin in cgi:
            ...
            if response.status == 200:
                var1 = var1 + 1
                print "%s %s" % ( "\n\n>>>" + host, "Pagina de administracion encontrada!")
                raw_input("Presiona una tecla pra continuar el escaner.\n")
            elif response.status == 404:
                var2 = var2
            elif response.status == 302:
                print "%s %s" % ("\n>>>" + host, "Posible pagina de administracion (302 - Redirect)")
    ...
    print "\n\t[!] Sesión Cancelada; Ocurrió un error. Compruebe la configuración de Internet"
